...
<body class="bg-gradient-to-br from-gray-50 to-blue-50 font-sans leading-normal tracking-normal">
    ...
    <style>
        ...
        .pengumuman-detail-image-wrapper {
            position: relative;
            overflow: hidden;
        }
        .pengumuman-detail-image-wrapper::before {
            content: '';
            position: absolute;
            top: 0;
            left: -75%;
        ...
        .pengumuman-detail-image-wrapper:hover::before {
            left: 120%;
            transition: left 0.6s cubic-bezier(0.4,0,0.2,1);
        }
        /* Animasi fade-in */
    ...
    <div class="container mx-auto px-4 py-12 relative">
        ...
        <div class="lg:col-span-8 animate-fade-in" style="animation-delay: 0.1s;">
            <article class="bg-white rounded-2xl shadow-lg overflow-hidden transition-all duration-300">
                <div class="pengumuman-detail-image-wrapper relative overflow-hidden rounded-t-2xl">
                    <img src="{{ asset('storage/' . $pengumuman->image_path) }}" 
                    ...
                    <div class="absolute bottom-0 left-0 right-0 p-6 text-white">
                        <div class="flex items-center space-x-2 mb-3">
                            <span class="bg-white/20 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-medium">{{ \Carbon\Carbon::parse($pengumuman->publish_date)->isoFormat('D MMMM YYYY') }}</span>
                            <span class="bg-white/20 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-medium">{{ $pengumuman->count_views }} Views</span>
                        </div>
                        <h2 class="text-3xl font-bold text-white drop-shadow-md">{{ $pengumuman->title }}</h2>
                    </div>
                </div>
                <div class="p-8 relative">
                    ...
                    <div class="mb-6 flex justify-between items-center">
                        <a href="{{ route('pengumuman.index') }}" class="text-sm text-blue-600 font-medium inline-flex items-center transition-colors duration-300 px-4 py-2 rounded-lg">
                        ...
                        <div class="flex items-center space-x-2 text-sm text-gray-500">
                            <div class="flex items-center">
                                ...
                                <span>{{ $pengumuman->count_views ?? 0 }} views</span>
                            </div>
                            <div class="flex items-center">
                                <i class="fas fa-clock mr-1"></i>
                                <span>{{ ceil(str_word_count(strip_tags($pengumuman->description)) / 200) }} min read</span>
                            </div>
                        </div>
                    </div>
                    <h1 class="text-4xl lg:text-5xl font-extrabold text-gray-900 mb-4 leading-tight hidden">{{ $pengumuman->title }}</h1>
                    <div class="flex flex-wrap items-center text-base text-gray-600 mb-8 space-x-6 animate-fade-in" style="animation-delay: 0.3s;">
                        ...
                        <div class="flex items-center bg-blue-50 px-3 py-1 rounded-lg">
                            ...
                            <span>{{ \Carbon\Carbon::parse($pengumuman->publish_date)->isoFormat('D MMMM YYYY') }}</span>
                        </div>
                        <div class="flex items-center bg-blue-50 px-3 py-1 rounded-lg">
                    ...
                    <div class="prose lg:prose-xl max-w-none text-gray-700 leading-relaxed border-t border-b border-gray-100 py-8 my-6 animate-fade-in" style="animation-delay: 0.5s;">
                        ...
                        <div class="not-prose mb-6 text-xl text-gray-500 font-serif italic border-l-4 border-blue-500 pl-4 py-2 bg-blue-50/50 rounded-r-lg">
                            {{ Str::words(strip_tags($pengumuman->description), 20) }}
                        </div>
                    ...
                    <div class="flex flex-wrap gap-2 mb-6">
                        @if(!empty($pengumuman->tags))
                            @foreach(explode(',', $pengumuman->tags) as $tag)
                                @if(!empty(trim($tag)))
                                    <a href="{{ route('pengumuman.index', ['search' => trim($tag)]) }}" class="bg-gray-100 text-gray-800 px-3 py-1 rounded-md text-sm font-medium transition-colors duration-300">#{{ trim($tag) }}</a>
                                @endif
                            @endforeach
                        @endif
                    </div>
                    ...
                    <div class="mt-6 border-t border-gray-100 pt-6 animate-fade-in" style="animation-delay: 0.8s;">
                        <p class="text-sm font-medium text-gray-700 mb-3">Bagikan pengumuman ini:</p>
                        <div class="flex flex-wrap gap-3">
                            ...
                            <a href="https://twitter.com/intent/tweet?url={{ url()->current() }}&text={{ $pengumuman->title }}" target="_blank" class="flex items-center justify-center w-10 h-10 bg-blue-100 text-blue-400 rounded-full transition-colors duration-300">
                                <i class="fab fa-twitter"></i>
                            </a>
                            <a href="https://wa.me/?text={{ $pengumuman->title }} {{ url()->current() }}" target="_blank" class="flex items-center justify-center w-10 h-10 bg-green-100 text-green-600 rounded-full transition-colors duration-300">
                                <i class="fab fa-whatsapp"></i>
                            </a>
                            <a href="https://www.linkedin.com/shareArticle?mini=true&url={{ url()->current() }}&title={{ $pengumuman->title }}" target="_blank" class="flex items-center justify-center w-10 h-10 bg-blue-100 text-blue-700 rounded-full transition-colors duration-300">
                                <i class="fab fa-linkedin-in"></i>
                            </a>
                            <a href="mailto:?subject={{ $pengumuman->title }}&body={{ url()->current() }}" class="flex items-center justify-center w-10 h-10 bg-red-100 text-red-600 rounded-full transition-colors duration-300">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8 animate-fade-in" style="animation-delay: 0.6s;">
                @if(isset($recentPosts) && $recentPosts->count() > 0)
                    @php
                        $prevPost = $recentPosts->where('publish_date', '<', $pengumuman->publish_date)->sortByDesc('publish_date')->first();
                        $nextPost = $recentPosts->where('publish_date', '>', $pengumuman->publish_date)->sortBy('publish_date')->first();
                        
                        // Jika tidak ada post sebelumnya/berikutnya, gunakan post terakhir/pertama sebagai fallback
                        if (!$prevPost && $recentPosts->count() > 1) {
                            $prevPost = $recentPosts->where('id', '!=', $pengumuman->id)->sortByDesc('publish_date')->first();
                        }
                        
                        if (!$nextPost && $recentPosts->count() > 1) {
                            $nextPost = $recentPosts->where('id', '!=', $pengumuman->id)->sortBy('publish_date')->first();
                        }
                    @endphp
                    
                    @if($prevPost)
                    <a href="{{ route('pengumuman.show', $prevPost->slug) }}" class="p-4 bg-white rounded-xl shadow-sm border border-gray-100 transition-all duration-300 flex items-center">
                        <div class="bg-blue-100 p-3 rounded-full mr-3 transition-colors duration-300">
                            <i class="fas fa-arrow-left"></i>
                        </div>
                        <div class="overflow-hidden">
                            <div class="text-xs text-gray-500 mb-1">Pengumuman Sebelumnya</div>
                    ...
                    @if($nextPost)
                    <a href="{{ route('pengumuman.show', $nextPost->slug) }}" class="p-4 bg-white rounded-xl shadow-sm border border-gray-100 transition-all duration-300 flex items-center justify-end text-right">
                        <div class="overflow-hidden">
                            <div class="text-xs text-gray-500 mb-1">Pengumuman Berikutnya</div>
                    ...
                    <div class="relative">
                        <form action="{{ route('pengumuman.index') }}" method="GET">
                            <div class="relative group">
                                <input type="text" name="search" placeholder="Cari pengumuman..." class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors duration-300">
                    ...
                    <div class="space-y-4">
                        @if(isset($recentPosts) && $recentPosts->count() > 0)
                            @foreach($recentPosts->take(5) as $post)
                                <a href="{{ route('pengumuman.show', $post->slug) }}" class="block">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @if(isset($allTags) && count($allTags) > 0)
                            @foreach($allTags as $tag)
                                <a href="{{ route('pengumuman.index', ['search' => $tag]) }}" class="px-3 py-1 bg-gray-100 text-gray-700 rounded-full text-sm transition-colors duration-300">
                                    #{{ $tag }}
                                </a>
                            @endforeach
                        @else
