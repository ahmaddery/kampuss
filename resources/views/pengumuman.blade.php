...
<head>
    ...
    <style>
        ...
        /* Efek untuk card pengumuman */
        .pengumuman-card {
            box-shadow: 0 4px 20px -5px rgba(0, 0, 0, 0.1);
        }
        .pengumuman-card-shadow {
            box-shadow: 0 4px 20px -5px rgba(0, 0, 0, 0.1);
        }
        .pengumuman-card-image {
        }
        .pengumuman-image-wrapper {
            position: relative;
            overflow: hidden;
        }
        .pengumuman-image-wrapper::before {
            content: '';
            position: absolute;
            top: 0;
            left: -75%;
        ...
        .pengumuman-image-wrapper:hover::before {
            left: 120%;
            transition: left 0.6s cubic-bezier(0.4,0,0.2,1);
        }
        }
...
<body class="bg-gradient-to-br from-gray-50 to-blue-50 min-h-screen">
    ...
    <div class="container mx-auto px-4 py-12 relative z-10">
        ...
        <div class="mb-12 max-w-3xl mx-auto relative">
            <form action="{{ route('pengumuman.index') }}" method="GET">
                <div class="relative group">
                    ...
                    <div class="relative">
                        <input type="text" name="search" placeholder="Cari pengumuman berdasarkan judul atau tag..." 
        ...
        <!-- Announcement Grid dengan tampilan modern -->
        <div id="pengumuman-terbaru" class="scroll-mt-16"></div>
        @if($pengumumans->count() > 0)
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
                @foreach($pengumumans as $pengumuman)
                    <div class="pengumuman-card bg-white rounded-2xl shadow-lg overflow-hidden flex flex-col border border-gray-100 relative pengumuman-card-shadow group" style="cursor:pointer;">
                        <a href="{{ route('pengumuman.show', $pengumuman->slug) }}" class="absolute inset-0 z-10" aria-label="Baca detail {{ $pengumuman->title }}" tabindex="0" style="display:block;"></a>
                        <!-- Ribbon untuk pengumuman terbaru jika publish date < 7 hari -->
                        @if(\Carbon\Carbon::parse($pengumuman->publish_date)->diffInDays(now()) < 7)
                        ...
                        <div class="relative overflow-hidden pengumuman-image-wrapper">
                            <img src="{{ asset('storage/' . $pengumuman->image_path) }}" 
                                 alt="{{ $pengumuman->title }}" 
                                 class="w-full h-56 object-cover pengumuman-card-image">
                            ...
                            <div class="absolute top-4 left-4 bg-blue-500 text-white px-4 py-1.5 rounded-full text-sm font-semibold shadow-md backdrop-blur-sm bg-opacity-90 capitalize">
                                @if(!empty($pengumuman->tags))
                                    <i class="fas fa-tag mr-1"></i> {{ trim(explode(',', $pengumuman->tags)[0]) }}
                                @else
                        ...
                        <div class="p-6 flex flex-col flex-grow">
                            <h2 class="text-2xl font-bold text-gray-900 mb-3 leading-tight hover:text-blue-600 transition-colors duration-300 line-clamp-2">{{ $pengumuman->title }}</h2>
                            ...
                            <p class="text-gray-700 flex-grow leading-relaxed line-clamp-3">{{ Str::limit(strip_tags($pengumuman->description), 130) }}</p>
                            ...
                            <div class="mt-6 pt-4 border-t border-gray-100 flex justify-between items-center bg-gradient-to-b from-white/0 to-gray-50/50 -mx-6 px-6">
                                ...
                                <a href="{{ route('pengumuman.show', $pengumuman->slug) }}" class="font-semibold text-blue-600 hover:text-blue-800 inline-flex items-center group bg-blue-50 px-4 py-2 rounded-lg transition-all duration-300 hover:bg-blue-100 relative btn-hover-effect">
            ...
            <div class="mt-8">
                <div class="bg-white rounded-2xl shadow-md p-10 text-center max-w-2xl mx-auto border border-gray-100 card-shadow">
                    <div class="w-24 h-24 bg-blue-100 rounded-full flex items-center justify-center mx-auto mb-6">
                        <i class="fas fa-bullhorn text-blue-500 text-3xl"></i>
                    </div>
                    <h3 class="text-2xl font-bold text-gray-800 mb-3">Tidak ada pengumuman yang ditemukan</h3>
                    <p class="text-gray-600 mb-6">Maaf, kami tidak dapat menemukan pengumuman yang sesuai dengan kriteria pencarian Anda.</p>
                    <div class="space-y-4">
                        <a href="{{ route('pengumuman.index') }}" class="inline-flex items-center justify-center px-6 py-3 bg-blue-600 text-white font-medium rounded-lg transition-all duration-300">
                        ...
                        <div class="flex flex-wrap justify-center gap-2 mt-2">
                            <a href="{{ route('pengumuman.index', ['search' => 'terbaru']) }}" class="px-4 py-2 bg-gray-100 rounded-full text-gray-700 text-sm">#terbaru</a>
                            <a href="{{ route('pengumuman.index', ['search' => 'kampus']) }}" class="px-4 py-2 bg-gray-100 rounded-full text-gray-700 text-sm">#kampus</a>
                            <a href="{{ route('pengumuman.index', ['search' => 'mahasiswa']) }}" class="px-4 py-2 bg-gray-100 rounded-full text-gray-700 text-sm">#mahasiswa</a>
                            <a href="{{ route('pengumuman.index', ['search' => 'kegiatan']) }}" class="px-4 py-2 bg-gray-100 rounded-full text-gray-700 text-sm">#kegiatan</a>
                        </div>
                    </div>
                </div>
                ...
                <div class="relative mt-16 max-w-4xl mx-auto">
                    ...
                    <div class="bg-white rounded-2xl shadow-md p-6 text-center border border-gray-100 card-shadow">
                        <h3 class="text-xl font-bold text-gray-800 mb-4">Pengumuman Akan Datang</h3>
                        <p class="text-gray-600 mb-4">Kami sedang menyiapkan pengumuman-pengumuman menarik untuk Anda. Kunjungi kembali halaman ini dalam beberapa hari.</p>
